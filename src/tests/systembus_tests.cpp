...
TEST_CASE("WRAM maps banks 7E and 7F as contiguous memory", "[unit]") {
    ...
    BusFollowResult read_first_bank =
        snes.system_bus->follow(snes.system_bus->plan(0x7E0001, BusAccessType::Read), 0, wram.getDeviceId());
    BusFollowResult read_second_bank =
        snes.system_bus->follow(snes.system_bus->plan(0x7F0002, BusAccessType::Read), 0, wram.getDeviceId());
