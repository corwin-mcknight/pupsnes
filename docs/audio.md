...
Synthesis uses the pinned **blargg snes_spc 0.9.0** DSP core with local interpolation and address-wrap adaptations. Host playback uses **miniaudio 0.11.25**, a bounded sample queue, and linear resampling to the selected device's rate. Only playback devices are opened. The source and licenses are recorded in the [DSP source notes](../src/third_party/snes_spc/README.md) and [miniaudio notes](../third_party/miniaudio/README.md).
