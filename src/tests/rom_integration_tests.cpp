...
std::string ParseQuotedString(const std::string& value,
...
uint32_t ParseUnsignedValue(const std::string& value,
                            const std::filesystem::path& path,
                            int line_number) {
  try {
...
RomStatus ParseRomStatus(const std::string& value,
...
GoalObservationKind ParseGoalKind(const std::string& value,
                                  const std::filesystem::path& path,
                                  int line_number) {
  if (value == "cpu_a8") {
...
void ApplyScenarioField(RomScenario& scenario, const std::string& key,
  ...
  if (key == "expected_current_status") {
    scenario.expected_current_status = ParseRomStatus(
        ParseQuotedString(value, path, line_number), path, line_number);
    return;
  }
...
void ApplyGoalField(GoalSpec& goal, const std::string& key,
                    const std::string& value, const std::filesystem::path& path,
                    int line_number) {
  if (key == "name") {
  ...
  if (key == "kind") {
    goal.kind = ParseGoalKind(ParseQuotedString(value, path, line_number), path,
                              line_number);
    return;
  }
  if (key == "address") {
...
void ValidateScenario(const RomScenario& scenario,
                      const std::filesystem::path& path) {
  ...
  if (scenario.cycle_budget == 0) {
    throw std::runtime_error(path.string() +
                             ": cycle_budget must be greater than zero");
  }
  if (scenario.goals.empty()) {
    throw std::runtime_error(path.string() +
                             ": at least one [goal] section is required");
  }
...
RomScenario LoadScenarioSpec(const std::filesystem::path& path) {
  ...
  while (std::getline(stream, line)) {
    ...
    const std::string key =
        Trim(std::string_view(trimmed).substr(0, equals_pos));
    const std::string value =
        Trim(std::string_view(trimmed).substr(equals_pos + 1));
...
std::vector<RomScenario> LoadScenarioSpecs() {
  const std::filesystem::path spec_dir =
      std::filesystem::path(PUPSNES_TEST_ROM_METADATA_DIR);
  std::vector<std::filesystem::path> spec_paths;

  for (const auto& entry :
       std::filesystem::recursive_directory_iterator(spec_dir)) {
...
uint32_t ReadObservationValue(const GoalSpec& goal, const CPU& cpu,
                              const WRAM& wram) {
  switch (goal.kind) {
...
std::string SummarizeResult(const RomExecutionResult& result) {
  ...
  out << "expected current status: "
      << StatusLabel(result.scenario.expected_current_status) << "\n";
  out << "observed status: " << StatusLabel(result.status) << "\n";
  out << "matched goals: " << result.matched_goals << "/"
      << result.goal_results.size() << "\n";
  out << "cpu time: " << result.snapshot.cpu_time
      << ", master time: " << result.snapshot.master_time << "\n";
  out << "cpu.a.low: "
      << FormatHex(static_cast<uint8_t>(result.snapshot.cpu_regs.A))
      << ", cpu.pc: " << FormatHex(result.snapshot.cpu_regs.PC)
      << ", micro-op index: "
      << static_cast<unsigned>(result.snapshot.cpu_micro_op_index) << "\n";
  if (!result.failure_summary.empty()) {
    out << "summary: " << result.failure_summary << "\n";
  }
  for (const GoalResult& goal : result.goal_results) {
    out << (goal.matched ? "[pass] " : "[fail] ") << goal.spec.name << ": "
        << goal.spec.description << " | " << ObservationLabel(goal.spec)
        << " expected " << FormatHex(goal.spec.expected_value) << ", got "
...
RomExecutionResult RunScenario(const RomScenario& scenario) {
  ...
  try {
    const std::filesystem::path rom_path =
        std::filesystem::path(PUPSNES_TEST_ROM_DIR) /
        (scenario.rom_name + ".sfc");
    ...
    snes.scheduler->ScheduleEvent(scenario.cycle_budget, nullptr,
                                  SchedulerPhase::kWakeSample,
                                  EventType::kDeviceBoundary);

    constexpr std::size_t kMaxSchedulerSteps = 256;
    std::size_t steps = 0;
    while (cpu.GetTime() < scenario.cycle_budget &&
           steps < kMaxSchedulerSteps) {
      snes.scheduler->Step();
      steps++;
    }
    ...
    if (cpu.GetTime() < scenario.cycle_budget) {
      ...
      result.failure_summary =
          "ROM run did not reach the configured cycle budget";
    } else {
