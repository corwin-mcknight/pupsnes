...
struct SchedulerTestAccess {
    static std::size_t eventQueueSize(const Scheduler& scheduler) { return scheduler.eventQueue.size(); }
    static SchedulerEvent peekNextEvent(const Scheduler& scheduler) { return scheduler.eventQueue.top(); }
    ...
    static std::size_t queuedRunEventsFor(const Scheduler& scheduler, device_id_t device_id) {
        ...
        while (!queue.empty()) {
            ...
            if (event.subphase == SchedulerPhase::Run && event.type == EventType::DeviceRun &&
